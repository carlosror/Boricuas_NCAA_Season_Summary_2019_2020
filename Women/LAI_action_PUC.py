...
players_list = []
players_names_list = []
with open("LAI_action_PUC.csv") as f:
  ...
  for line in f:
    ...
    if (player_name in players_names_list): 
      ...
      image_filename = "./Puerto_Rico/LAI_action/" + player_name.replace(" ", "_") + str(player_name_count + 1) + ".jpg"
    else:
      image_filename = "./Puerto_Rico/LAI_action/" + player_name.replace(" ", "_") + ".jpg"
            
    maldonado_justiniano_lopez = "Aleiehs Maldonado (PUCPR) & Khris Justiniano & María López"
    if (current_line_list[0] == maldonado_justiniano_lopez):
      player_name = maldonado_justiniano_lopez
      image_filename = "./Puerto_Rico/LAI_action/" + maldonado_justiniano_lopez.replace(" ", "_") + ".jpg"
    
    maldonado_arguello_burgos = "Aleiehs Maldonado (PUCPR) & Alejandra Argüello & Nydiaris Burgos"
    if (current_line_list[0] == maldonado_arguello_burgos):
      player_name = maldonado_arguello_burgos
      image_filename = "./Puerto_Rico/LAI_action/" + maldonado_arguello_burgos.replace(" ", "_") + ".jpg"
    
    lugo_maldonado_arguello = "Joyce_Lugo & Aleiehs Maldonado & Alejandra Argüello (UAGM)"
    if (current_line_list[0] == lugo_maldonado_arguello):
      player_name = lugo_maldonado_arguello
      image_filename = "./Puerto_Rico/LAI_action/" + lugo_maldonado_arguello.replace(" ", "_") + ".jpg"
...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  for page in range(num_pages):
    ...
    setFillColor("PUC", top_rect); setLineColor("PUC", top_rect)
    bottom_rect = createRect(0, 756, 612, 36)
    setFillColor("PUC", bottom_rect); setLineColor("PUC", bottom_rect)
    ...
    setText("Pontificia Universidad Católica", page_header)
    ...
    for row in range(4):
      for col in range(2):
        ...
        if current_player[5] == "NCAA DII":
          player_division = createImage(league_x, league_y + 20, 25, 25)
        ...
        if (current_player[0] in [maldonado_justiniano_lopez, maldonado_arguello_burgos, lugo_maldonado_arguello]):
          logo_name = "Pontificia_Universidad_Católica"
        ...
        if (current_player[0] in [maldonado_justiniano_lopez, maldonado_arguello_burgos, lugo_maldonado_arguello]): setFontSize(7, player_name)
        ...
        if (current_player[0] in [maldonado_arguello_burgos, lugo_maldonado_arguello]):
          logo_name_2 = "Universidad_Ana_G_Méndez"
        ...
        if (current_player[0] in [maldonado_justiniano_lopez]):
          logo_name_2 = "Recinto_Universitario_de_Mayagüez"
          if (school_logos_dict[logo_name_2] < 0.7):
            logo_width = 33.0
            logo_height = min(logo_width * school_logos_dict[logo_name_2], 28)
          else:
