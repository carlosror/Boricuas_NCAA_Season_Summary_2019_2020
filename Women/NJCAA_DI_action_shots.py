...
players_list = []
players_names_list = []
with open("NJCAA_DI_action.csv") as f:
  ...
  for line in f:
    ...
    if (full_name[1] == "de" or full_name[1] == "La"):
      ...
      if (player_name in players_names_list): 
        ...
        image_filename = "./NJCAA_DI_action/" + first_name + "_" + full_name[1] + "_" + full_name[2] + "_" + str(player_name_count + 1) + ".jpg"
      else:
        image_filename = "./NJCAA_DI_action/" + first_name + "_" + full_name[1] + "_" + full_name[2] + ".jpg"
    else:
      player_name = first_name + " " + first_last_name
      if (player_name in players_names_list): 
        player_name_count = sum([1 for plyr in players_names_list if plyr == player_name])
        image_filename = "./NJCAA_DI_action/" + first_name + "_" + first_last_name + "_" + str(player_name_count + 1) + ".jpg"
      else:
        image_filename = "./NJCAA_DI_action/" + first_name + "_" + first_last_name + ".jpg"
      
    jimenez_cordova = "Alanis Jiménez & María Córdova"; orsini_almeda = "Melanie Orsini & Carolina Almeda"
    hcc = "Hillsborough Community College"
    if (current_line_list[0] == jimenez_cordova):
      player_name = jimenez_cordova
      image_filename = "./NJCAA_DI_action/" + jimenez_cordova.replace(" ", "_") + ".jpg"
    elif (current_line_list[0] == orsini_almeda):
      player_name = orsini_almeda
      image_filename = "./NJCAA_DI_action/" + orsini_almeda.replace(" ", "_") + ".jpg"
    elif (current_line_list[0] == hcc):
      player_name = hcc
      image_filename = "./NJCAA_DI_action/" + hcc.replace(" ", "_") + ".jpg"
...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  for page in range(num_pages):
    ...
    setFillColor("White", center_rect); setLineColor("NJCAA Gray", center_rect)
    
    page_header = createText(36, 9, 540, 36)
    setText("NJCAA Division I Action", page_header)
    ...
    for row in range(4):
      for col in range(2):
        ...
        if (current_player[0] in [jimenez_cordova, orsini_almeda, hcc]):
