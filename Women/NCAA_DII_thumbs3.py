...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  for page in range(num_pages):
    for col in range(num_cols):
      ...
      thumb_college_w = 110
      thumb_state_w = 66
      thumb_name_frame = createText(270 * col + 36 + 4 + thumb_w + 4, 36 + 8, thumb_name_w, 756)
      thumb_college_frame = createText(270 * col + 36 + 4 + thumb_w + 4 + thumb_name_w , 36 + 8, thumb_college_w, 756)
      thumb_state_frame = createText(270 * col + 36 + 4 + thumb_w + thumb_name_w + 4 + thumb_college_w, 36 + 8 + 8, thumb_state_w, 756)
      for row in range(num_rows):
        # if ((row + 1) % 2) == 0:
          # gray_row = createRect((270 * col + 36), (36 + row * 36), 276, 36)
          # setFillColor("NJCAA Gray", gray_row); setLineColor("NJCAA Gray", gray_row)
        ...
        # thumb_name_w = 60
        # thumb_name_frame = createText(thumb_name_x, thumb_y + 8, thumb_name_w, 22)
