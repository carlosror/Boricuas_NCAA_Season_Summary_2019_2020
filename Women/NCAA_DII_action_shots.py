...
players_list = []
players_names_list = []
with open("NCAA_DII_action.csv") as f:
  ...
  for line in f:
    ...
    if (full_name[1] == "de" or full_name[1] == "La"):
      ...
      if (player_name in players_names_list): 
        ...
        image_filename = "./NCAA_DII_action/" + first_name + "_" + full_name[1] + "_" + full_name[2] + "_" + str(player_name_count + 1) + ".jpg"
      else:
        image_filename = "./NCAA_DII_action/" + first_name + "_" + full_name[1] + "_" + full_name[2] + ".jpg"
    else:
      player_name = first_name + " " + first_last_name
      if (player_name in players_names_list): 
        player_name_count = sum([1 for plyr in players_names_list if plyr == player_name])
        image_filename = "./NCAA_DII_action/" + first_name + "_" + first_last_name + "_" + str(player_name_count + 1) + ".jpg"
      else:
        image_filename = "./NCAA_DII_action/" + first_name + "_" + first_last_name + ".jpg"
      
    gonzalez_serra = "Claudia González & Andrea Serra"; rabassa_hostos_rivera = "Camila Rabassa & Militza de Hostos & Tanializ Rivera"
    alejandro_cespeded_gonzalez = "Nahomie Alejandro & Paula Céspedes & Talía González"
    rivera_dominguez = "Claudia Rivera & Nicole Domínguez"
    if (current_line_list[0] == gonzalez_serra):
      player_name = gonzalez_serra
      image_filename = "./NCAA_DII_action/" + gonzalez_serra.replace(" ", "_") + ".jpg"
    elif (current_line_list[0] == rabassa_hostos_rivera):
      player_name = rabassa_hostos_rivera
      image_filename = "./NCAA_DII_action/" + rabassa_hostos_rivera.replace(" ", "_") + ".jpg"
    elif (current_line_list[0] == alejandro_cespeded_gonzalez):
      player_name = alejandro_cespeded_gonzalez
      image_filename = "./NCAA_DII_action/" + alejandro_cespeded_gonzalez.replace(" ", "_") + ".jpg"
    elif (current_line_list[0] == rivera_dominguez):
      player_name = rivera_dominguez
      image_filename = "./NCAA_DII_action/" + rivera_dominguez.replace(" ", "_") + ".jpg"
...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  for page in range(num_pages):
    ...
    setText("NCAA Division II Action", page_header)
    ...
    for row in range(4):
      for col in range(2):
        ...
        if (current_player[0] in [gonzalez_serra, rabassa_hostos_rivera, alejandro_cespeded_gonzalez, rivera_dominguez]):
        ...
        if (current_player[0] in [rabassa_hostos_rivera, alejandro_cespeded_gonzalez]):
