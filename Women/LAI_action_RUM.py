...
players_list = []
players_names_list = []
with open("LAI_action_RUM.csv") as f:
  ...
  for line in f:
    ...
    arvelo_ramos = "Camila Arvelo & Gabriela Ramos"
    ayala_bautista = "Jovyrelis Ayala & Johanna Bautista"
    lopez_justiniano = "María López & Khris Justiniano"
    # if (current_line_list[0] == ayala_bautista):
      # player_name = ayala_bautista
      # image_filename = "./Puerto_Rico/LAI_action/" + ayala_bautista.replace(" ", "_") + ".jpg"
...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  for page in range(num_pages):
    ...
    setFillColor("RUM", top_rect); setLineColor("RUM", top_rect)
    bottom_rect = createRect(0, 756, 612, 36)
    setFillColor("RUM", bottom_rect); setLineColor("RUM", bottom_rect)
    ...
    setText("Recinto Universitario de Mayagüez", page_header)
    ...
    for row in range(4):
      for col in range(2):
        ...
        if (current_player[0] in [arvelo_ramos, ayala_bautista, lopez_justiniano]):
        ...
        if (current_player[0] in [arvelo_ramos, ayala_bautista, lopez_justiniano]): setFontSize(13, player_name)
