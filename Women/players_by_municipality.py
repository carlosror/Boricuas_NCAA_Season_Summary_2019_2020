...
escudos_dict = {}
with open("./Escudos/filesizes_jpg.csv") as f:
  for line in f:
    current_line_list = line.split(",")
    escudos_dict[current_line_list[0]] = float(current_line_list[2]) / float (current_line_list[1])

muni_dict = {}
with open("./all_players_order_municip.csv") as f:
  ...
  for line in f:
    ...
    if current_line_list[7] not in muni_dict.keys():
      muni_dict[current_line_list[7]] = [single_player_list]
    else:
      muni_dict[current_line_list[7]].append(single_player_list)
...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  page_num_rows = 18
  page_num_municip = 0
  escudo_h = 44; escudo_x = 36 + 4
  y_offset = 0
  municipalities = list(muni_dict.keys())
  municipalities = sorted(municipalities)
  for municip in municipalities:
    num_players = len(muni_dict[municip])
    ...
    municip_banner_pos = 36 + 18 + y_offset
    municip_banner = createRect(36, municip_banner_pos, 540, 54)
    setFillColor("White", municip_banner); setLineColor("NJCAA Blue", municip_banner)
    escudo_y = municip_banner_pos + (54.0 - escudo_h) / 2.0
    escudo_w = escudo_h / escudos_dict[municip.replace(" ", "_")]
    muni_escudo = createImage(escudo_x, escudo_y, escudo_w, escudo_h)
    loadImage("./Escudos/"+ municip.replace(" ", "_") + ".jpg", muni_escudo); setScaleImageToFrame(1, 1, muni_escudo)
    
    municip_name = createText(escudo_x + escudo_w + 8, municip_banner_pos + (54.0 - 24.0) / 2.0 + 3.0, 200, 28)
    setText(municip, municip_name)
    setFont("Playball Regular", municip_name); setFontSize(24, municip_name); setTextColor("NJCAA Blue", municip_name)
    
    player_count = 0
    players_list_size = len(muni_dict[municip])
    for col in range(2):
      thumb_name_w = 60
      thumb_college_w = 110
      thumb_state_w = 67
      thumb_name_frame = createText(270 * col + 36 + 4 + thumb_w + 4, municip_banner_pos + 54 + 8, thumb_name_w, num_rows * 36)
      thumb_college_frame = createText(270 * col + 36 + 4 + thumb_w + 4 + thumb_name_w , municip_banner_pos + 54 + 8, thumb_college_w, num_rows * 36)
      thumb_state_frame = createText(270 * col + 36 + 4 + thumb_w + 4 + thumb_name_w + thumb_college_w, municip_banner_pos + 54 + 8 + 6, thumb_state_w, num_rows * 36)
