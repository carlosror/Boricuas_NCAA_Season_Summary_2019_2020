...
players_list = []
with open("NCAA_NJCAA_NAIA_beach_volleyball_players_2019_2020.csv") as f:
  next(f)
  for line in f:
...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  for page in range(1, num_pages + 1):
    ...
    setFont("Asimov Print C", years1); setFontSize(11, years1); setTextAlignment(ALIGN_CENTERED, years1)
    setFont("Asimov Print C", years2); setFontSize(11, years2); setTextAlignment(ALIGN_CENTERED, years2)
    ...
    for row in range(1, num_rows + 1):
      ...
      for col in range(1, num_cols + 1):
        ...
        mugshot_text = createText(pic_x_coord, pic_y_coord + mugshot_h + 5, mugshot_w, 3 * player_name_text_height)
        ...
        if player_hometowm != "":
          ...
          setFont("Playball Regular", mugshot_text)
          selectText(name_class_length, len(player_hometowm), mugshot_text); setFontSize(player_hometown_text_height - 2, mugshot_text)
        setTextColor("NJCAA Blue", mugshot_text)
        setLineSpacing(11, mugshot_text)
