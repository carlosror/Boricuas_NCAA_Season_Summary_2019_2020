...
library(extrafont)
library(png)
aic_winloss <- read.csv("AIC_article_2019_2020/AIC_record_2019_2020.csv")
aic_logo <- readPNG("C:/Users/cesargb/Documents/Boricuas_NCAA/Season_Summary_2019_2020/Women/School_Logos/American_International_College.png")
aic_team <- readPNG("C:/Users/cesargb/Documents/Boricuas_NCAA/Season_Summary_2019_2020/Women/AIC_article_2019_2020/AIC_team.png")
...
p <- ggplot(data=aic_winloss, aes(x=c(1:31), y=Winning_Pctg)) + geom_line(size=1.2) + geom_point(size = 3) + 
     geom_text(aes(label = Result, x=c(1:31), y=Winning_Pctg), position = position_dodge(width = 0.75), vjust = -0.6, colour = "gold", fontface = "bold") + 
     theme_plot() + 
     ...
     geom_text(x=3, y=0.22, label="2-14", size = 14, family = "OLD SPORT 02 ATHLETIC NCV") + 
     ...
     geom_text(x=9, y=0.22, label="0.192", size = 14, family = "OLD SPORT 02 ATHLETIC NCV") + 
     annotate(geom = "text", x=9, y=0.175, label = "opponents\nhitting %") + 
     geom_text(x=24, y=0.32, label="12-3", size = 14, family = "OLD SPORT 02 ATHLETIC NCV", color = "gold") + 
     annotate(geom = "text", x=24, y=0.275, label = "in final\n15 matches", color = "gold") +
     geom_text(x=30, y=0.32, label="0.125", size = 14, family = "OLD SPORT 02 ATHLETIC NCV", color = "gold") + 
     annotate(geom = "text", x=30, y=0.275, label = "opponents\nhitting %", color = "gold") + 
     geom_text(x=27, y=0.22, label="14-5", size = 14, family = "OLD SPORT 02 ATHLETIC NCV", color = "gold") + 
     annotate(geom = "text", x=27, y=0.175, label = "overall record\nwhen opponents\nhitting % < 0.180", color = "gold") + 
