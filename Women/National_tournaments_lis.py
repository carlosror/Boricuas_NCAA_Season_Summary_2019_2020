...
with open("National_tournaments.csv") as f:
  ...
  for line in f:
    ...
    single_player_list = [player_name, player_school, tournament, outcome]
...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  for page in range(num_pages):
    ...
    setText("National Tournaments Participants", page_header)
    ...
    player_school_frame_width = 140; player_school_frame_xpos = player_name_frame_xpos + player_name_frame_width
    player_school_frame = createText(player_school_frame_xpos, offset + 5, player_school_frame_width, num_rows * row_height)
    player_tournament_frame_width = 225; player_tournament_frame_xpos = player_school_frame_width + player_school_frame_xpos
    player_tournament_frame = createText(player_tournament_frame_xpos, offset + 5, player_tournament_frame_width, num_rows * row_height)
    player_outcome_frame_width = 80; player_outcome_frame_xpos = player_tournament_frame_width + player_tournament_frame_xpos
    player_outcome_frame = createText(player_outcome_frame_xpos, offset + 5, player_outcome_frame_width, num_rows * row_height)
    ...
    player_tournament_frame_header = createText(player_tournament_frame_xpos, offset - header_height + 4, player_tournament_frame_width, header_height)
    insertText("Tournament", -1, player_tournament_frame_header)
    setFont("Asimov Print C", player_tournament_frame_header); setFontSize(12, player_tournament_frame_header)
    setTextAlignment(ALIGN_CENTERED, player_tournament_frame_header); setTextColor("NJCAA Blue", player_tournament_frame_header);
    player_outcome_frame_header = createText(player_outcome_frame_xpos, offset - header_height + 4, player_outcome_frame_width, header_height)
    insertText("Outcome", -1, player_outcome_frame_header)
    setFont("Asimov Print C", player_outcome_frame_header); setFontSize(12, player_outcome_frame_header)
    setTextAlignment(ALIGN_CENTERED, player_outcome_frame_header); setTextColor("NJCAA Blue", player_outcome_frame_header);
      
    for row in range(num_rows):
      current_player = players_list[player_count]
      insertText(current_player[0] + 2 *"\n", -1, player_name_frame)
      insertText(current_player[1] + 2 *"\n", -1, player_school_frame)
      insertText(current_player[2] + 2 *"\n", -1, player_tournament_frame)
      insertText(current_player[3] + 2 *"\n", -1, player_outcome_frame)
      player_count += 1
    ...
    setFont("Asimov Print C", player_tournament_frame); setFontSize(8.5, player_tournament_frame); setTextAlignment(ALIGN_CENTERED, player_tournament_frame)
    setTextColor("NJCAA Blue", player_tournament_frame); setLineSpacing(9.005, player_tournament_frame)
    setFont("Asimov Print C", player_outcome_frame); setFontSize(8.5, player_outcome_frame); setTextAlignment(ALIGN_CENTERED, player_outcome_frame)
    setTextColor("NJCAA Blue", player_outcome_frame); setLineSpacing(9.005, player_outcome_frame)
    ...
    if player_count == num_players: 
      ...
      for xpos in [36, player_school_frame_xpos, player_tournament_frame_xpos, player_outcome_frame_xpos, 576]:
