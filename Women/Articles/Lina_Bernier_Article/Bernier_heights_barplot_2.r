...
theme_plot <- function(...) {
  ...
  theme( 
    ...
    plot.title = element_text(hjust = 0.0, vjust = 10, size = 20), 
    plot.subtitle = element_text(hjust = 0.0, vjust = 5, size = 14),
...
p <- ggplot(dig_rankings_500) + geom_bar(aes(x=Height), fill="steelblue4", width=0.5, position = position_dodge(width=0.7)) + 
     ...
     xlab("Height") +  ylab("Number of players") + 
     ...
     theme_plot() + 
     ggtitle(label = "Standing tall", subtitle = "Number of players with 500+ digs in NCAA Division I, by height\nAcademic Year 2019-2020") + 
...
ggsave(plot = p, filename = "Bernier_heights.png", width = 9.5, height = 9.5 * 627 / 925, device = "png", units = "in")